...
public class Document {
	...
	// Wir ziehen jegliche "Leerzeichen" in betracht, nur " " hätte aber schon
	// gereicht
	...
	public WordCount[] getWordCountArray() {
		...
		// Falls Wörter doppelt vorkommen, müssen wir das Array kürzen
	...
	public static WordCount[] equalizeWordCountArray(WordCount[] first, WordCount[] second) {
		...
		// Falls alle Wörter jeweils einzigartig sind haben wir höchstens so viele
		// `WordCount`-Objekte
		WordCount[] temp = new WordCount[first.length + second.length];

		// Kopiert alle Elemente aus dem ersten Array in temp
		System.arraycopy(first, 0, temp, 0, first.length);
		int size = first.length;

		// Einzigartige Wörter aus dem zweiten Array hinzufügen
		for (WordCount wordCount : second) {
			// Wir verwenden findIndex, um zu überprüfen, ob das Wort bereits im
			// 'first'-Array vorhanden ist
